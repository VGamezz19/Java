...
public class Clinic {
    ...
    public void addNurse() {
        elements[comptaElements] = Nurse.addNurse();
        comptaElements++;
    }
    ...
    public void addPatient() {
        elements[comptaElements] = Patient.addPatient();
        comptaElements++;
    }
    ...
    public void addDepartment() {
        elements[comptaElements] = Department.addDepartment();
        comptaElements++;
    }
    ...
    public Integer selectElement(Integer tipusElement) {
        ...
        for (int i = 0; i < comptaElements; i++) {
            ...
            if (elements[i] instanceof Nurse && laClasse == 2) {
                if (((Nurse) elements[i]).getNif().equals(codiSel)) {
                    indexSel = i;
                    break;
                }
            }
            if (elements[i] instanceof Patient && laClasse == 3) {
                if (((Patient) elements[i]).getCode().equals(codiSel)) {
                    indexSel = i;
                    break;
                }
            }
            if (elements[i] instanceof Department && laClasse == 4) {
                if (((Department) elements[i]).getCode().equals(codiSel)) {
                    indexSel = i;
                    break;
                }
            }
    ...
    public void addElementDepartment(Integer tipusElement) {
        ...
        if (indexSelDepartment >= 0) {
            departmentsel = (Department) this.getElements()[indexSelDepartment];
        } else {
            System.out.println("\nNo existeix aquest departament");
        }
        Integer indexSel = this.selectElement(tipusElement);
        if (indexSel >= 0) {
            departmentsel.addElementDepartment(this.getElements()[indexSel]);
        } else {
